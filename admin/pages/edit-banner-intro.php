...
// 1. Lấy dữ liệu cũ dựa trên ID từ URL
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    // Bạn cần đảm bảo trong model BannerIntro đã có hàm getById($id)
    // Nếu chưa có, hãy thêm vào model: SELECT * FROM banner_introduct WHERE banner_id = ?
    $sql = Db::$connection->prepare("SELECT * FROM banner_introduct WHERE banner_id = ?");
    $sql->bind_param("i", $id);
    $sql->execute();
    $banner = $sql->get_result()->fetch_assoc();
...
if (isset($_POST['btn-edit'])) {
    $image = $_POST['image'];
    $is_view = isset($_POST['is_view']) ? 1 : 0; // Xử lý nút gạt On/Off

    if ($introModel->update($id, $image, $is_view)) {
        header("Location: banner-intro.php?success=updated");
    } else {
      ...
      <div class="col-12">
        <div class="card shadow-lg">
          <div class="card-header pb-0">
            <h3 class="font-weight-bolder text-info text-gradient">Sửa Banner #<?php echo $id; ?></h3>
          </div>
          <div class="card-body">
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <input type="text" name="image" class="form-control" value="<?php echo $banner['image']; ?>" required>
                  </div>
                </div>
                ...
                <div class="col-md-12 mt-3">
                  ...
                  <div class="mb-3">
                    <img src="<?php echo $banner['image']; ?>" class="border-radius-lg shadow" style="max-height: 200px;">
                  </div>
                </div>

                <div class="col-md-12 mt-2">
                  <label>Trạng thái hiển thị (On/Off)</label>
                  <div class="form-check form-switch ps-0">
                    <input class="form-check-input mt-1 ms-auto" type="checkbox" name="is_view" value="1" id="is_view" <?php echo ($banner['is_view'] == 1) ? 'checked' : ''; ?>>
                    <label class="form-check-label ms-3" for="is_view">Bật/Tắt hiển thị trên website</label>
                  </div>
                </div>
              </div>

              <div class="d-flex justify-content-end mt-4">
                <a href="banner-intro.php" class="btn btn-light m-0">Hủy</a>
                <button type="submit" name="btn-edit" class="btn bg-gradient-info ms-2 m-0">Cập nhật thay đổi</button>
              </div>
            </form>
          </div>
        </div>

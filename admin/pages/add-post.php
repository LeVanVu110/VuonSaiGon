...
$categories = $catPostModel->getCategoriesPage(1, 100); // Lấy tất cả danh mục bài viết

if (isset($_POST['btn-add'])) {
    $postModel = new Posts();
    // Xử lý danh mục (cat_ids là một mảng)
    $cat_ids = $_POST['cat_ids'] ?? []; 
    $result = $postModel->add($_POST['title'], $_POST['slug'], $_POST['image'], $_POST['summary'], $_POST['content'], $_POST['author'], $cat_ids);
    if($result) header("Location: post.php?success=add");
}
      ...
      <form method="POST">
        <div class="row">
          <div class="col-md-8">
            ...
            <textarea name="summary" class="form-control mb-3" rows="3"></textarea>
          ...
          <div class="col-md-4">
            ...
            <label>Tác giả</label>
            <input type="text" name="author" class="form-control mb-3" value="<?= $_SESSION['user']['gmail'] ?>">
            <label>Danh mục (Giữ Ctrl để chọn nhiều)</label>
            <select name="cat_ids[]" class="form-control mb-3" multiple style="height: 150px;">
        ...
        <div class="d-flex justify-content-end">
          <button type="submit" name="btn-add" class="btn bg-gradient-info">Đăng bài viết</button>
        </div>
      </form>
    </div>
  </div>

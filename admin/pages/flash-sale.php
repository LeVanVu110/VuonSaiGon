...
$events = $flashModel->getFlashSalesPage($page, $perPage, $keyword);
$totalPages = ceil($flashModel->getTotal($keyword) / $perPage);
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="g-sidenav-show bg-gray-100">
    <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 fixed-start" id="sidenav-main"
        ...
        <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main" style="height: calc(100vh - 200px);">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="../pages/flash-sale.php">
                        <div
                            class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <div class="card mb-4 shadow-lg">
    <div class="card-header pb-0 d-flex justify-content-between align-items-center">
      <h6>Danh sách Sự kiện Flash Sale</h6>
      <div class="d-flex">
        <form method="GET" class="me-3">
          <input type="text" name="keyword" class="form-control" placeholder="Tìm tên sự kiện..." value="<?= htmlspecialchars($keyword) ?>">
        </form>
        <a href="add-flash-sale.php" class="btn bg-gradient-info btn-sm mb-0">+ Thêm sự kiện</a>
      </div>
    </div>
    <div class="card-body px-0 pt-0 pb-2 text-start">
      <div class="table-responsive p-0">
        <table class="table align-items-center mb-0 text-center">
          <thead>
            <tr>
              <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Tên sự kiện</th>
              <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Thời gian bắt đầu</th>
              <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Thời gian kết thúc</th>
          ...
          <tbody>
            ...
            <tr>
              <td class="text-start"><h6 class="mb-0 text-sm ps-3"><?= $e['name'] ?></h6></td>
              <td><span class="text-xs font-weight-bold"><?= date('d/m/Y H:i', strtotime($e['start_time'])) ?></span></td>
              <td><span class="text-xs font-weight-bold"><?= date('d/m/Y H:i', strtotime($e['end_time'])) ?></span></td>
              <td>
                <span class="badge badge-sm <?= $e['is_active'] ? 'bg-gradient-success' : 'bg-gradient-secondary' ?>">
                  <?= $e['is_active'] ? 'Đang chạy' : 'Tạm dừng' ?>
                </span>
              </td>
              <td class="align-middle">
                <a href="edit-flash-sale.php?id=<?= $e['id'] ?>" class="text-info font-weight-bold text-xs me-3">Sửa</a>
                <a href="delete-flash-sale.php?id=<?= $e['id'] ?>" class="text-danger font-weight-bold text-xs" onclick="return confirm('Xóa sự kiện này?')">Xóa</a>
              </td>
            </tr>
            <?php endforeach; ?>
          </tbody>

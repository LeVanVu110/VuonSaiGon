...
// Tăng giới hạn lấy bài viết để tránh thiếu sót
...
if (isset($_POST['btn-add'])) {
    ...
    if ($titleModel->add($title, $post_id, $content, $parent_id)) {
        echo "<script>window.location.href='article-title.php?success=add';</script>";
        exit();
    } else {
...
<body class="g-sidenav-show bg-gray-100">
    ...
    <div class="card shadow-lg col-12">
        <div class="card-header pb-0">
            <h6>Thêm mục lục bài viết mới</h6>
            <?php if(isset($error)): ?>
            ...
            <form method="POST">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label font-weight-bold">1. Chọn bài viết gốc</label>
                        <select name="post_id" id="post_id" class="form-select form-control" required onchange="loadParents(this.value)">
                            <option value="">-- Chọn bài viết --</option>
                            <?php foreach($posts as $p): ?>
                                <option value="<?= $p['post_id'] ?>"><?= htmlspecialchars($p['title']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <small class="text-muted">Chọn "Mục chính" nếu đây là đề mục lớn.</small>
                    </div>

                    <div class="col-md-12 mb-3">
                        <label class="form-label font-weight-bold">3. Tiêu đề mục chi tiết</label>
                        <input type="text" name="title" class="form-control" placeholder="VD: 1. Cách trồng hoa..." required>
                    </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="article-title.php" class="btn btn-secondary me-2">Quay lại</a>
                    <button type="submit" name="btn-add" class="btn bg-gradient-info">Lưu thông tin</button>
                </div>
            </form>
        </div>
    </div>
...
function loadParents(postId) {
    ...
    if (postId !== "") {
        $.ajax({
            ...
            error: function() {
                alert("Lỗi tải danh sách mục cha!");
            }
        });
    } else {

...
$parents = $catModel->getParentCategories(); // Lấy danh sách để chọn lại danh mục cha

if(isset($_POST['btn-edit'])) {
    $result = $catModel->updateCategory($id, $_POST['name'], $_POST['slug'], $_POST['parent_id'], $_POST['sort_order'], $_POST['description'], $_POST['image_url']);
    if($result) header("Location: category.php?success=updated");
}
?>
<!DOCTYPE html>
<html lang="en">
      ...
      <form method="POST">
        <div class="row">
          <div class="col-md-6">
            ...
            <input type="text" name="name" class="form-control mb-3" value="<?php echo $cat['name']; ?>" required>
          </div>
          <div class="col-md-6">
            ...
            <select name="parent_id" class="form-control mb-3">
              ...
              <?php foreach($parents as $p): ?>
                <option value="<?php echo $p['id']; ?>" <?php echo ($cat['parent_id'] == $p['id']) ? 'selected' : ''; ?>><?php echo $p['name']; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-md-6">
            <label>Đường dẫn (Slug)</label>
            <input type="text" name="slug" class="form-control mb-3" value="<?php echo $cat['slug']; ?>" required>
          </div>
          <div class="col-md-6">
            <label>Thứ tự hiển thị</label>
            <input type="number" name="sort_order" class="form-control mb-3" value="<?php echo $cat['sort_order']; ?>">
          </div>
          <div class="col-12">
            <label>Link ảnh đại diện</label>
            <input type="text" name="image_url" class="form-control mb-3" value="<?php echo $cat['image_url']; ?>">
            <label>Mô tả</label>
            <textarea name="description" class="form-control mb-3" rows="3"><?php echo $cat['description']; ?></textarea>
          </div>
        </div>
        <div class="d-flex justify-content-end">
          <a href="category.php" class="btn btn-light m-0">Hủy</a>
          <button type="submit" name="btn-edit" class="btn bg-gradient-info ms-2 m-0">Cập nhật</button>
        </div>
      </form>
    </div>
  </div>

...
$categories = $catPostModel->getCategoriesPage(1, 100); // Lấy danh sách danh mục để chọn

// Chuyển chuỗi danh mục "1,4" thành mảng [1, 4] để kiểm tra selected
$selected_cats = !empty($post['selected_cats']) ? explode(',', $post['selected_cats']) : [];

if (isset($_POST['btn-edit'])) {
    $cat_ids = $_POST['cat_ids'] ?? [];
    if ($postModel->update($id, $_POST['title'], $_POST['slug'], $_POST['image'], $_POST['summary'], $_POST['content'], $_POST['author'], $cat_ids)) {
        header("Location: post.php?success=updated");
        exit();
    }
}
?>
...
<body class="g-sidenav-show bg-gray-100">
   <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 fixed-start" id="sidenav-main"
        ...
        <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main" style="height: calc(100vh - 200px);">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link  active" href="../pages/post.php">
                        <div
                            class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                            <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <div class="card shadow-lg col-12">
    <div class="card-header pb-0">
      <h3 class="font-weight-bolder text-info text-gradient">Sửa bài viết #<?= $id ?></h3>
    </div>
    <div class="card-body">
      <form method="POST">
        <div class="row">
          <div class="col-md-8 text-start">
            ...
            <label>Tóm tắt</label>
            <textarea name="summary" class="form-control mb-3" rows="3"><?= $post['summary'] ?></textarea>
            
            <label>Nội dung bài viết</label>
            <textarea name="content" class="form-control mb-3" rows="12" required><?= $post['content'] ?></textarea>
          </div>
          
          <div class="col-md-4 text-start">
            <label>Slug</label>
            <input type="text" name="slug" class="form-control mb-3" value="<?= $post['slug'] ?>" required>
            
            <label>Ảnh đại diện (URL)</label>
            <input type="text" name="image" class="form-control mb-2" value="<?= $post['image'] ?>">
            <img src="<?= $post['image'] ?>" class="img-fluid border-radius-lg mb-3 shadow">
            
            <label>Tác giả</label>
            <input type="text" name="author" class="form-control mb-3" value="<?= $post['author'] ?>">
            
            <label>Danh mục (Chọn nhiều)</label>
            <select name="cat_ids[]" class="form-control mb-3" multiple style="height: 180px;">
            ...
            <p class="text-xs text-secondary">Giữ phím Ctrl để chọn hoặc bỏ chọn nhiều danh mục.</p>
          </div>
        </div>
        
        <div class="d-flex justify-content-end mt-4">
          <a href="post.php" class="btn btn-light m-0 me-2">Hủy</a>
          <button type="submit" name="btn-edit" class="btn bg-gradient-info m-0">Cập nhật bài viết</button>
        </div>
      </form>
    </div>
  </div>
